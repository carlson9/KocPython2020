...
#from a file
sm = pystan.StanModel(file = 'KocPython2019/10.Stan/8schools.stan')
...
os.chdir('KocPython2019/10.Stan')
...
generated quantities {
  real y_new;
  real new_mu;
  new_mu = normal_rng(mu_a, sigma_a) + mean(u)*b[1] + mean(x)*b[2] + mean(x_mean)*b[3];
  y_new = normal_rng(new_mu, sigma_y);
}
"""

contextual_pred_data2 = {'N': len(log_radon),
                          'J': len(n_county),
                          'county': county+1, # Stan counts starting at 1
                          'u': u,
                          'x_mean': x_mean,
...
contextual_pred_fit2 = pystan.stan(model_code=contextual_pred2, data=contextual_pred_data2, 
                                         iter=1000, chains=2)

contextual_pred_fit2.plot('y_new');
...
"""
logitFit3 = pystan.stan(model_code=turnout_model_gen_inter, data=turnout_data,                                         iter=1000, chains=2, control = {'adapt_delta': .99, 'max_treedepth': 20})

#TODO: Allow the economic slopes and intercept to vary by race


turnout_model_gen_slopes = """
data {
    int<lower=0> N;
    int<lower=0> K;
    matrix[N,K] X;
...
transformed parameters {
    vector[N] eta;
    for(n in 1:N) eta[n] = a + X[n,1:(K-1)]*B + X[n,K]*b[race[n]] + re[race[n]];
}
model {
    a ~ normal(0,10);
    re_mu ~ normal(0,10);
...
logitFit4 = pystan.stan(model_code=turnout_model_gen_slopes, data=turnout_data,                                         iter=1000, chains=2, control = {'adapt_delta': .99, 'max_treedepth': 20})
